...
from .models import (
    GroupProxy, 
    Client, Doctor, News, Employee,
    Partner, Vacancy,
    GlossaryTerm, Cabinet, Specialization,
    Service, Appointment, PromoCode, Review,
    UserProfile, CompanyInfo,Cart,CartItem,Payment
)
...
class DoctorAdmin(admin.ModelAdmin):
    ...
    list_display = ('full_name', 'cabinet', 'experience')
    ...
    fieldsets = (
        ...
        ('Профессиональная информация', {
            'fields': ('cabinet', 'specializations', 'services', 'license_number')
        }),
        ('Даты', {
            'fields': ('start_date',),
            'classes': ('collapse',)
...
class CompanyInfoAdmin(admin.ModelAdmin):
    list_display = ("name", "founded", "employees")
